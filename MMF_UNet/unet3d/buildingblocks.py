...
from MMF_UNet.unet3d.se import ChannelSELayer3D, ChannelSpatialSELayer3D, SpatialSELayer3D
...
class Decoder(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, conv_kernel_size=3, scale_factor=(2, 2, 2), basic_module=DoubleConv,
                 conv_layer_order='gcr', num_groups=8, mode='nearest', padding=1, upsample=True, is3d=True, name='UNet3D'):
...
def create_encoders(in_channels, f_maps, basic_module, conv_kernel_size, conv_padding, layer_order, num_groups,
                    pool_kernel_size, is3d, first_need_down=False):
    ...
    for i, out_feature_num in enumerate(f_maps):
        if i == 0:
            ...
            if first_need_down:
                encoder = nn.Conv3d(in_channels, out_feature_num, kernel_size=(3, 7, 7), stride=(1, 3, 3), padding=(1, 1, 1))
            else:
...
def create_decoders(f_maps, basic_module, conv_kernel_size, conv_padding, layer_order, num_groups, is3d, name='UNet3D'):
